...
from py_replay_bg.model.model_step_equations_t1d import identify_multi_meal, identify_multi_meal_exercise
from py_replay_bg.model.model_step_equations_t1d import model_step_equations_multi_meal, model_step_equations_multi_meal_exercise


class T1DModelMultiMeal:
    """
    A class that represents the type 1 diabetes model in a multi-meal scenario.
    ...
    def __init__(self, data, bw, yts=5, glucose_model='IG', u2ss=None, X0=None, previous_data_name=None, exercise=False, environment=None, identification_method='mcmc'):
        ...
        self.yts = yts  # Measurement sampling time
        ...
        if self.previous_data_name is not None:
            prefix = 'draws' if identification_method == 'mcmc' else 'map'
            with open(os.path.join(environment.replay_bg_path, 'results', prefix,
                                   prefix+'_' + previous_data_name + '.pkl'), 'rb') as file:
    ...
    def simulate(self, rbg_data, modality, rbg, sensors=None):
        ...
        if is_replay:
            ...
            # Run simulation
            self.x = identify_multi_meal(self.tsteps, self.x, self.A, self.B, bolus_delayed, basal_delayed,
    ...
    def __log_likelihood_exercise(self, theta, rbg_data):
        """
        Internal function that computes the log likelihood of unknown parameters.
