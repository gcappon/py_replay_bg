...
class CGM:
    ...
    def connect_new_cgm(self):
        ...
        mu = np.array([0.94228767821000314341972625697963, 0.0049398821141803427384187052950892, -0.0005848748565491275084801681138913, 6.382602204050874306062723917421, 1.2604070417357611244568715846981, -0.4022228938823663169088717950217, 3.2516360856114072674927228945307])
        sigma = np.array([[0.013245827952891258902368143424155,  -0.0039513025350735725416129184850433,  0.00031276743283791636970891936186945,   0.15717912467153988265167186000326,  0.0026876560011614997885986966252858,  -0.0028904633825263671524641306831427, -0.0031801707001874032418320403792222],
        ...
        while (~ stable) or (output_noise_SD > max_output_noise_SD):
            ...
            stable = (cgm_error_parameters[5] >= -1) and (cgm_error_parameters[5] <= ( 1- np.abs(cgm_error_parameters[4]) - toll))
            
            #Compute the output noise standard deviation
            output_noise_SD = np.sqrt(cgm_error_parameters[6]**2 / (1- cgm_error_parameters[4]**2/(1-cgm_error_parameters[5]) - cgm_error_parameters[5]*(cgm_error_parameters[4]**2/(1-cgm_error_parameters[5])+cgm_error_parameters[5])))
    ...
    def measure(self, IG, t):
        ...
        IGs = (self.cgm_error_parameters[0] + self.cgm_error_parameters[1]*t + self.cgm_error_parameters[2]*t**2)*IG + self.cgm_error_parameters[3]
